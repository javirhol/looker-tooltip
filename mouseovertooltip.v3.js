...
function setIconAnchor(divIcon, tooltipPosition) {
  const pos = String(tooltipPosition || "auto").toLowerCase();

  if (pos === "left") {
    divIcon.style.justifyContent = "flex-end";
  } else if (pos === "right") {
...
Tooltip.DELAY = 150;
Tooltip.OFFSET = 15;
Tooltip.EDGE_PADDING = 8;

Tooltip.prototype = {
  Show: function (text, x, y, tcolor, bcolor, tfont, tsize, bwidth, bstyle, bradius, bgcolor, position) {
    ...
    const tooltipWidth = this.shadow.offsetWidth;
    const tooltipHeight = this.shadow.offsetHeight;

    let finalX;
    let finalY;

    const preferredPosition = String(position || "auto").trim().toLowerCase();

    const placeRight = x + Tooltip.OFFSET;
    const placeLeft = x - tooltipWidth - Tooltip.OFFSET;

    if (preferredPosition === "left") {
      finalX = placeLeft;
      if (finalX < Tooltip.EDGE_PADDING) {
        finalX = placeRight;
      }
    } else if (preferredPosition === "right") {
      finalX = placeRight;
      if (finalX + tooltipWidth > viewportWidth - Tooltip.EDGE_PADDING) {
        finalX = placeLeft;
      }
    } else {
      finalX = placeRight;
    ...
    finalX = clamp(
      finalX,
      Tooltip.EDGE_PADDING,
      Math.max(Tooltip.EDGE_PADDING, viewportWidth - tooltipWidth - Tooltip.EDGE_PADDING)
    );

    finalY = y + Tooltip.OFFSET;
    if (finalY + tooltipHeight > viewportHeight - Tooltip.EDGE_PADDING) {
      finalY = y - tooltipHeight - Tooltip.OFFSET;
    }

    finalY = clamp(
      finalY,
      Tooltip.EDGE_PADDING,
      Math.max(Tooltip.EDGE_PADDING, viewportHeight - tooltipHeight - Tooltip.EDGE_PADDING)
    );

    this.shadow.style.left = `${finalX}px`;
    this.shadow.style.top = `${finalY}px`;
  ...
  Schedule: function (targetElement, event, tColor, bColor, tFont, tSize, bWidth, bStyle, bRadius, bgColor, position) {
...
function isRenderReady() {
  ...
  const docReady = docEl && docEl.clientWidth > 0 && docEl.clientHeight > 0;
  const bodyReady = body && body.clientWidth > 0 && body.clientHeight > 0;

  return !!(docReady || bodyReady);
}

function disconnectReadyWatchers() {
  if (readyObserver) {
...
function renderNow(data) {
  ...
  const st = {
    ...
    tooltipPosition: normalizeIconValue(getStyleVal(data.style?.tooltipposition, "auto")).toLowerCase()
  };

  setIconAnchor(divIcon, st.tooltipPosition);
...
function tryRenderWhenReady(data) {
  lastData = data;

  if (isRenderReady()) {
    disconnectReadyWatchers();
    renderNow(data);
    return;
  }
